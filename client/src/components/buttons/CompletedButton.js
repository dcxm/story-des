...
const CompletedButton = ({ item, tooltip, style, updateItem, type }) => {
    const [icon, setIcon] = useState();

    useEffect(() => setIcon(item.completed ? <DoneIcon /> : <CloseIcon />), [item])

    const onClick = () => {
        updateItem(type === 'novels' ? 'novel' : 'shortStory', type === 'novels' ? 'novels' : 'shortStories', { id: item.id, completed: !item.completed })
    }
